<template>
  <aside class="flex h-full w-64 flex-shrink-0 flex-col border-r border-border bg-card">
    <div class="flex h-16 items-center gap-2 border-b border-border px-4">
      ...
      <span class="text-lg font-bold text-foreground">SlideAI</span>
    </div>

    <div class="space-y-1 p-3">
      <button :class="btnClass(view === 'new')" @click="$emit('new')">
        <Plus class="h-4 w-4" /> 新建生成
      </button>
      <button :class="btnClass(view === 'explore')" @click="$emit('explore')">
        <Compass class="h-4 w-4" /> 探索 Explore
      </button>
    </div>

    <div class="flex min-h-0 flex-1 flex-col px-3">
      <p class="px-2 py-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">我的历史</p>
      <div class="min-h-0 flex-1 space-y-0.5 overflow-y-auto">
        ...
        <p v-else-if="!myProjects.length" class="px-2 py-2 text-xs text-muted-foreground/70">暂无记录</p>
        <button
        ...
        >
          ...
          <span class="truncate">{{ p.name || p.title || '未命名项目' }}</span>
        </button>
      </div>
    </div>
    ...
    <div class="border-t border-border p-3">
      <div class="flex items-center justify-between gap-2">
        <span class="truncate text-sm text-muted-foreground">{{ nickName || '已登录' }}</span>
        <button
          title="退出登录"
