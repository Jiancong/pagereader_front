<template>
  <div class="mx-auto max-w-6xl">
    <div class="mb-6">
      <h2 class="text-2xl font-bold text-foreground">探索作品</h2>
      <p class="mt-1 text-sm text-muted-foreground">看看大家做的书籍分析，点开可基于它再生成一份</p>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
      ...
      >
        <div class="aspect-[3/4] w-full overflow-hidden bg-secondary/40">
          ...
          />
          <div v-else class="flex h-full w-full items-center justify-center text-xs text-muted-foreground">无封面</div>
        </div>
        <div class="flex flex-1 flex-col p-3">
          <p class="line-clamp-2 text-sm font-medium text-foreground">{{ item.name || item.nameEn || '未命名' }}</p>
    ...
    <p v-if="!loading && !items.length && !error" class="py-12 text-center text-sm text-muted-foreground">暂无作品</p>
  </div>
</template>
