...
function buildPptRelatedSearchMessage(term: string, pptTitle?: string): string {
  ...
  if (locale.value.startsWith("zh")) {
    return title
      ? `关于演示文稿《${title}》中的内容，请详细解释「${q}」的含义、背景和其在文中的具体指代。`
      : `请详细解释「${q}」的含义与背景。`;
  }
  return title
    ? `In the presentation "${title}", please explain in detail what "${q}" means and its context.`
    : `Please explain in detail what "${q}" means and its background.`;
}
